@PrimaryFlood @Admitted @US50285
Feature: PF Admitted - Copy Flood Blocks to Newly Created Flood Node - IL
  As a product manager, I want to copy some of the flood related blocks,
  fields and functionality from several UI screens to the newly created flood node
  so we can start offering Primary Flood endorsements to our members.

  @TC97498 @TC97499
  Scenario: US50285 - Verify flood tree node for multiple locations new business - IL_001 - 09/07/2025
    ...
    * I verify block "excess flood" is "visible" on page
    ...
    When I navigate to Homeowner Policy Page and add new risk location with basic details only
      ...
      | 2324 N LINCOLN PARK | Chicago | IL    | 60614 | Homeowner     | 7500000 |           | 10,000 |            |
  ...
  @TC97498 @TC97499
  Scenario: US50285 - Verify flood tree node for multiple locations endorsement - IL_001 - 09/06/2025
  ...
  @TC97498 @TC97499
  Scenario: US50285 - Verify flood tree node for multiple locations renewal - IL_001 - 09/07/2024
  ...
  Scenario Outline: US50285 - Verify flood tree node is not visible  - "<state>" - "<effectiveDate>"
