...
Feature: PF Admitted - New Flood Questions Function - IL & CO (Mandatory questions in elevation and primary flood coverage blocks)
  ...
  @TC104383
  Scenario: US54360 - Verify that new questions are optional to rate and mandatory to bind for new business policies and mandatory to rate for newly added risk locations on the existing policies (NB/END) - CO - 09/07/2025
  ...
  @TC104388
  Scenario: US54360 - Verify that new questions are optional to rate and mandatory to bind only for new business policies and mandatory for newly added risk locations on the existing policies (NB/RNW) - CO - 09/07/2025
  ...
  @TC104393
  Scenario: US54360 - Verify that new questions are not mandatory to rate for the existing locations on Endorsement after NB PD Filing date (NB/END) - CO - 09/06/2025
    ...
    * I take screenshot "Primary flood coverage not mandatory question - END (first location)"
    ...
    Then I navigate to Homeowner Policy Page and add new risk location with basic details except Flood Page
      ...
      | 101 Main Street | Platteville | CO    | 80651 | Homeowner     | 7500000 | Unlimited | 10,000 | 2%         |1|
    * I click order property details button
    * I navigate to "Platteville CO" page
    * I navigate to "Flood Coverage" page
  ...
  @TC104394
  Scenario: US54360 - Verify that new questions are not mandatory to rate for the existing locations on Renewal after REN PD Filing date (NB/RNW) - CO - 09/07/2024
    ...
    * I take screenshot "Primary flood coverage not mandatory question - RNW (first location)"
    ...
    When I click "No" radio button for distant label "Would you like to quote Non-Admitted Primary Flood (in order to bind Non-Admitted Primary Flood this question must be answered 'yes')?"
    * I click "No" radio button for label "Flood Coverage Extension (DIC)"
